...
def doWork( args ):
    ...
    # parse nucmer file
    NP = NucMerParser()
    # objects big_vs_small, small vs big
    uni_vs_smil_dir = {}

    with open(args.uni_vs_smil_coords, 'r') as fh:
        for hit in NP.readNuc(fh):
            smillie = hit[NP._ID_2] 
            univec = hit[NP._ID_1]
            try: 
                uni_vs_smil_dir[smillie] += [univec] 
            except KeyError:
                uni_vs_smil_dir[smillie] = [univec]
    for key in uni_vs_smil_dir.keys():
        print key
...
if __name__ == '__main__':
    ...
    #parser.add_argument('-s','--smil_vs_uni_coords', help="Big vs small nucmer coords file")
    parser.add_argument('-u','--uni_vs_smil_coords', help="Small vs big nucmer coords file")
