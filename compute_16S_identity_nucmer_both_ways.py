...
def doWork( args ):
    ...
    for i in range(len(fasta_urls)):                    # Mikes example commands for running the script
        for j in range(len(fasta_urls)): # +1 and -1 to the for loops, means that only the bottom half of the triangle will be compared. 
            cmds.append("nucmer %s %s --mum --coords -p %s" % (fasta_urls[i], fasta_urls[j], "%sv%s" %(fasta_urls[i],fasta_urls[j])))
